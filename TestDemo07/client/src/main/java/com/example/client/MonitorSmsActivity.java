...
public class MonitorSmsActivity extends AppCompatActivity {
    ...
    private static class SmsGetObserver extends ContentObserver{
        ...
        public void onChange(boolean selfChange, @Nullable Uri uri) {
            ...
            // 安卓7.0以上系统，点击标记为已读，也会调用一次,路径为mUri===content://sms
            // 收到一条短信都是uri后面都会有确定的一个数字，对应数据库的_id，比如上面的20
            if (uri == null){
                return;
            }
            //屏蔽content://sms/raw和content://sms
            if (uri.toString().contains("content://sms/raw") || uri.toString().equals("content://sms")){return;}
            //通过内容解析器获取符合条件的结果集游标
            Cursor cursor = mContent.getContentResolver().query(uri,new String[]{"address","body","date"},null,null,"date DESC");
            //只要最顶上最新的短信
            if (cursor.moveToNext()){
                ...
                Log.d("boge",String.format("sender:%s,content:%s",sender,content));
            }
            cursor.close();
        }
    }
